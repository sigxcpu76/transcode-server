...
init([]) ->
    {ok, TRef} = timer:send_interval(timer:seconds(1),
				     doit),
...
terminate(_Reason, State) ->
    {ok, cancel} = timer:cancel(State#state.tref), ok.
...
all_changed() ->
    [M
     || {M, Fn} <- code:all_loaded(), is_list(Fn),
	is_changed(M)].
...
reload(Module) ->
    ...
    case code:load_file(Module) of
      ...
      {error, Reason} ->
	  io:format(" fail: ~p.~n", [Reason]), error
    end.
